        ...
        return {
            ...
            file_picker_callback: (cb, value, meta) => {
                ...
                window.renderFileManager({
                    onInsert: (data) => {
                        // Extract to object files & dirPath from data object
                        const { files, dirPath } = data;
                        const fileName = files[0].split('/').reverse()[0].split('\\').reverse()[0].replace(/\.[^/.]+$/, "");
                        const fileNameBeautified = fileName.split('-').join(' ').split('_').join(' ');
                        cb(files[0], { title: fileNameBeautified, alt: fileNameBeautified });
                    }
                });
        ...
        inlineTools() {
            ...
            tools.forEach(tool => {
                ...
                resOuter.forEach(outer => {
                    ...
                    const editorWrapper = tool.querySelector(`.${variables.selectorClass}-editor-wrapper`);
                    const inlineToolLink = document.createElement('a');
                    const inlineToolCallback = this.config.inlineFunctions[obj.callback];

                    inlineToolLink.title = obj.title;
                    inlineToolLink.setAttribute('uk-tooltip', obj.title);
                    inlineToolLink.href = '#';
                    inlineToolLink.className = `${this.selectorClass}-editor-tootls ${obj.trigger}`;
                    inlineToolLink.innerHTML = obj.icon;

                    if (typeof inlineToolCallback === 'function') {
                        inlineToolLink.addEventListener('click', (event) => {
                            event.preventDefault();
                            inlineToolCallback.call(inlineToolLink, event);
                        });
                    }

                    editorWrapper.appendChild(inlineToolLink);
                });
